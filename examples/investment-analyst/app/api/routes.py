...
def get_crypto_prices():
    ...
    result = run_async(skills_service.execute("crypto_prices", symbols="bitcoin,ethereum,solana,cardano,dogecoin"))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data


@router.get("/market/crypto/{coin_id}")
def get_crypto_detail(coin_id: str):
    """Get detailed crypto data."""
    result = run_async(skills_service.execute("crypto_detailed", coin_id=coin_id))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data
...
def get_market_indices():
    ...
    result = run_async(skills_service.execute("market_indices"))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data
...
def get_commodities():
    ...
    result = run_async(skills_service.execute("commodities"))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data
...
def get_forex():
    """Get forex rates."""
    result = run_async(skills_service.execute("forex", pairs="EUR/USD,GBP/USD,USD/JPY"))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data
...
def get_fear_greed():
    ...
    result = run_async(skills_service.execute("fear_greed"))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data
...
def get_trending_crypto():
    ...
    result = run_async(skills_service.execute("crypto_trending"))
    if not result.success:
        raise HTTPException(status_code=500, detail=result.error)
    return result.data
